...
SqlCommand InterPreter::select_from(string sql)
{
    ...
    if (sql.find("from") == -1) //对于from有错误的话，直接报错
    {
        temp.set_command_type(SQL_ERROR);
        return temp;
    }
    ...
    {
        /*取出需要的属性列名称*/
        string attrs = sql.substr(0, sql.find("from"));
        //去除两端空格
        attrs.erase(0, attrs.find_first_not_of(" "));
        attrs.erase(attrs.find_last_not_of(" ") + 1);
        //取出属性列
        while (attrs.length() != 0)
        {
            temp.push_select_attr(attrs.substr(0, attrs.find(",")));
            if (attrs.find(",") != -1)
                attrs = attrs.substr(attrs.find(",") + 1);
            else
                break;
        }
    }
    ...
    /*判断是否有表名代取*/
    string tableInfo = sql.substr(0, sql.find("where"));
    //去除空格
    tableInfo.erase(tableInfo.find_last_not_of(" ") + 1);
    //有逗号就是多表
    if (tableInfo.find(",") != -1)
    {
        //取出多表
        ...
        {
            temp.push_select_table(tableInfo.substr(0, tableInfo.find(",")));
            if (tableInfo.find(",") != -1)
                tableInfo = tableInfo.substr(tableInfo.find(",") + 1);
            else
                break;
        }
        sql = sql.substr(sql.find("where"));
    }
    else
    {
        //单表
    ...
    if (get_first_word(sql, " ") == "where")
    {
        ...
        while (get_first_word(sql, " ") != ";")
        {
            ...
            sql = get_split_string_value(sql); //这部分主要处理掉了条件里面的左值，右值和中间的比较值
            int re_flag = -1;
            if (is_digit(left_value) && is_digit(right_value))
            {
                re_flag = re_judge(left_value, right_value, op_value);
            }
...
\
            ...
            else
            {
                if (re_flag == 1)
                {
                    if (temp.and_or_flag == 2)
                    {
                        temp.clear_command_left_vector();
                        temp.clear_command_op_vector();
                        temp.clear_command_right_vector();
                    }
                }
                else
                {
                    if (temp.and_or_flag == 1)
                    {
                        flag_and_error = 1;
                    }
                }
    ...
    {
        temp.set_command_type(SQL_NO_RESULT);
    }
...
SqlCommand InterPreter::create_table(string sql)
{
    ...
    while (get_first_word(sql, " ;").length() != 0) //一直读到末尾
    {
        ...
        else
        {
            ...
            else if (expression.find("Date") == 0)
            {
                temp.push_value_type(-2);
            }
...
int InterPreter::is_digit(string s)
{
    int i = 0, flag = 1;
    if (s[0] == '-' || s[0] == '+')
        i = 1;
...
int InterPreter::re_judge(string left_data, string right_data, string op)
{
    if (op == ">")
        return atof(left_data.c_str()) > atof(right_data.c_str());
    if (op == "<")
        return atof(left_data.c_str()) < atof(right_data.c_str());
    if (op == ">=")
        return atof(left_data.c_str()) >= atof(right_data.c_str());
