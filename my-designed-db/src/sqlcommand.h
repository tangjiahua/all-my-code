...
#define SQL_NO_RESULT 1016

class SqlCommand
{
public:
    int and_or_flag = 1;                 //1是and 2是or
    ...
    vector<int> value_type;              //0代表整型, -1代表小数, 1-255代表的是char型的长度，-2代表日期
    vector<string> select_attr;          //需要投影的属性名
    vector<string> select_table;         //多表查询的表名

    void push_select_table(string str);
    vector<string> get_select_table();

    void push_select_attr(string str);
    vector<string> get_select_attr();
    ...
    void clear_command_left_vector();
    void clear_command_op_vector();
    void clear_command_right_vector();
