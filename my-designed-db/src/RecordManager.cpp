    ...
    while (!meet_tail(query_addr, chunk, table_name))
    {
        ...
        else if (and_or_flag == 2)
        {
            ...
            for (i = 0; i < Condition_vec.size(); ++i)
            {
                ...
                case INT:
                    memcpy(&inttmp, chunk->data + query_addr + offset, length);
                    flag_tmp = Condition_vec[i].ifRight(inttmp);
                    break;
                case FLOAT:
                    memcpy(&floattmp, chunk->data + query_addr + offset, length);
                    flag_tmp = Condition_vec[i].ifRight(floattmp);
                    break;
                case DATE:
                case CHAR:
                    memcpy(chtmp, chunk->data + query_addr + offset, length);
                    chtmp[length] = '\0';
                    strtmp = string(chtmp);
                    flag_tmp = Condition_vec[i].ifRight(strtmp);
                    break;
                }
                if (flag_tmp)
                {
...
bool Condition::ifRight(string content)
{
    ...
    case Condition::OPERATOR_LIKE:
        return isMatch(content, myContent);
        break;
    default:
        return true;
        break;
...
bool Condition::isMatch(string s, string p)
{
    int SIndex = 0, PIndex = 0, SStart = 0, PStart = 0;
    while (SIndex < s.length())
    {
        //--s和p一一匹配
        if (s[SIndex] == p[PIndex] || p[PIndex] == '_')
        {
            SIndex++;
            PIndex++;
        }
    ...
    //--SIndex==s.length(),字符串s已经遍历完成，字符串p剩下的部分必须都为*，否则返回false
