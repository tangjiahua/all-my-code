...
void SqlCommand::push_select_table(string str)
{
    select_table.push_back(str);
}
vector<string> SqlCommand::get_select_table()
{
    return select_table;
}
void SqlCommand::push_select_attr(string str)
{
    select_attr.push_back(str);
}
vector<string> SqlCommand::get_select_attr()
{
    return select_attr;
}
...
void SqlCommand::clear_command_left_vector()
{
    for (int i = 0; i < command_left_vector.size(); i++)
        command_left_vector.pop_back();
}

void SqlCommand::clear_command_op_vector()
{
    for (int i = 0; i < command_op_vector.size(); i++)
        command_op_vector.pop_back();
}

void SqlCommand::clear_command_right_vector()
{
    for (int i = 0; i < command_right_vector.size(); i++)
        command_right_vector.pop_back();
}
